...
class Viessmann(SmartPlugin):
    ...
    def connect(self):
        self._lock.acquire()
        if self._connected and self._serialconnection:
            return True
        try:
            self.log_info('Connecting ...')
            self._serialconnection = serial.Serial()
            self._serialconnection.baudrate = self._bautrate
            self._serialconnection.parity = self._parity
            self._serialconnection.bytesize = self._bytesize
            self._serialconnection.stopbits = self._stopbits
            self._serialconnection.port = self._serialport
            self._serialconnection.timeout = 1
    ...
    def read_bytes(self, length):
        ...
        while True:
            ch = self._serialconnection.read()
            #self.log_debug('read_bytes: Read {}'.format(ch))
            if len(ch) == 0:
                #self.logger.debug('read_bytes: End read')
                return total
            total += ch
            if len(total) >= length:
                #self.logger.debug('read_bytes: End read')
    ...
    def parse_item(self, item):
        ...
        if self.has_iattr(item.conf, 'viess_read'):
            ...
            if (self.has_iattr(item.conf, 'viess_init') and bool(self.get_iattr_value(item.conf, 'viess_init')) is True):
                ...
                self.log_debug('CommandCodes should read at init: {}'.format(self._init_cmds))
            ...
            if (self.has_iattr(item.conf, 'viess_read_cycle')):
                ...
                self.log_debug('CommandCodes should read cyclic: {}'.format(self._cyclic_cmds))
    ...
    def update_item(self, item, caller=None, source=None, dest=None):
        ...
        if caller != 'viessmann' and self.has_iattr(item.conf, 'viess_update'):
            value = item()
            if value is True:
                self.log_debug('Reading of all value/items has been requested')
    ...
    def send_command(self, packet, packetlen_response):
        ...
        try:
            ...
            #if e[0] == 5:
            #    self.log_err('Try to solve error by reconnecting')
            #    self.disconnect()
            #    time.sleep(1)
            #    self.connect()
        #finally:
    ...
    def run(self):
        self.connect()
        self.alive = True
        self.scheduler_add('viessmann-init', self.send_init_commands, prio=4, cycle=600, offset=2)
        maxloops = 20
        loops = 0
    ...
    def send_init_commands(self):
        try:
            ...
            if shortestcycle != -1:
                ...
                self.scheduler_add('viessmann-cyclic', self.handle_cyclic_cmds, cycle=workercycle, prio=4, offset=4)
