...
class Viessmann(SmartPlugin):
    ...
    def __init__(self, sh, *args, **kwargs):
        ...
        if self._protocol in commands.controlset and self._protocol in commands.errorset and self._protocol in commands.unitset and self._protocol in commands.returnstatus and self._protocol in commands.setreturnstatus:
            ...
            self.log_err('Sets for Protocol {} could not be found!'.format(self._protocol))
            return None
    ...
    def _connect(self):
        ...
        try:
            ...
            self.log_err('Could not _connect to {}; Error: {}'.format(self._serialport, e))
            return False
        finally:
            self._lock.release()
    ...
    def _initCommunication(self):
        ...
        self.log_info('Communication initialized: {}'.format(is_initialized))
    ...
    def update_item(self, item, caller=None, source=None, dest=None):
        if self.alive and caller != self.get_shortname():
            ...
            if self.has_iattr(item.conf, 'viess_send'):
                ...
                if not self._create_write_command(commandname, value):
                ...
                if self.has_iattr(item.conf, 'viess_read') and self.has_iattr(item.conf, 'viess_read_afterwrite'):
                    ...
                    if readcommandname is not None and readafterwrite is not None:
                        ...
                        time.sleep(aw)
                        self._create_read_command(readcommandname)
                    ...
                    if trigger is None:
                        ...
                        for triggername in trigger:
                            ...
                            if triggername is not None and readafterwrite is not None:
                                ...
                                time.sleep(tdelay)
                                self._create_read_command(triggername)
    ...
    def send_cyclic_cmds(self):
        ...
        # recall function for sh.py scheduler thread

        # check if another cyclic cmd run is still active
        if self._cyclic_update_active:
            self.log_warn('Triggered cyclic command read, but previous cyclic run is still active. Check device and cyclic configuration (too much/too short?)')
            return
        ...
        for commandcode in list(self._cyclic_cmds.keys()):
            ...
            if entry['nexttime'] <= currenttime:
                ...
                self._create_read_command(commandname)
                entry['nexttime'] = currenttime + entry['cycle']
                read_items += 1
        self._cyclic_update_active = False
        self.log_debug("cyclic command read took {:.1f} seconds for {} items".format(time.time() - currenttime, read_items))
    ...
    def _create_read_command(self, commandname):
    ...
    def _create_write_command(self, commandname, value=None):
        ...
        if commandunit == 'BA':
            ...
            try:
                ...
                self.log_err('Value {} not defined in operating modes for device {}'.format(value, self._heating_type))
                return False
    ...
    def _parse_response(self, response):
        ...
        if commandcode in self._params.keys() and responsedatacode == 1 and responsetypecode != 3:
            ...
            self.log_debug('Rawdatabytes formatted: {} and unformatted: {}'.format(self.bytes2hexstring(rawdatabytes), rawdatabytes))
    ...
    def _serialnumber_decode(self, serialnummerbytes):
        ...
        for byte in range(0, len(serialnummerbytes)):
            serialnumber += (serialnummerbytes[byte] - 48) * 10 ** bytes
