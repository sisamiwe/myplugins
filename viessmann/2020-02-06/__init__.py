...
#                self._params[commandcode] = {'commandname': commandname, 'item': item}
#            elif item not in self._params[commandcode]['item']:
#                self._params[commandcode]['commandname'].append(commandname)
#                self._params[commandcode]['item'].append(item)
            # Loaded params '{'2323': {'commandname': ['Betriebsart_A1M1'], 'items': [Item: viessmann.heizkreis_a1m1.betriebsart.betriebsart]}, '2301': {'commandname': ['Aktuelle_Betriebsart_A1M1'], 'items': [Item: viessmann.heizkreis_a1m1.betriebsart.betriebsart_aktuell]}, '2302': {'commandname': ['Sparbetrieb_A1M1'], 'items': [Item: viessmann.heizkreis_a1m1.betriebsart.sparbetrieb]}}'
            
            key = 1
            if commandcode in self._params:
                self._params[commandcode][key] = {'item': item, 'commandname': commandname}
            else:
                self._params[commandcode] = {key: {'item': item, 'commandname': commandname}}
            # Loaded params '{'2323': {1: {'item': Item: viessmann.heizkreis_a1m1.betriebsart.betriebsart, 'commandname': 'Betriebsart_A1M1'}}, '2301': {1: {'item': Item: viessmann.heizkreis_a1m1.betriebsart.betriebsart_aktuell, 'commandname': 'Aktuelle_Betriebsart_A1M1'}}}'
            ...
            if (self.has_iattr(item.conf, 'viess_init') and bool(self.get_iattr_value(item.conf, 'viess_init')) is True):
                ...
                if commandcode not in self._init_cmds and not self.has_iattr(item.conf, 'viess_read_cycle'):
    ...
    def create_write_command(self, commandname, value=None):
        ...
        if set_allowed is True:
            ...
            if value is not None and value != '':
                ...
                self.log_debug('No value handed over')
        else:
    ...
    def send_command(self, packet, packetlen_response):
        try:
            ...
            if self._initialized is True:
                ...
                while self.alive and len(response_packet) < packetlen_response:
                    try:
                        bytestoreceive = packetlen_response - len(response_packet)
    ...
    def parse_response(self, response):
        ...
        if commandcode in self._params.keys() and responsedatacode == 1 and responsetypecode != 3:
            ...
            for i in range(0, len(self._params[commandcode]['item'])):
                item = self._params[commandcode]['item'][i]
                ...
                if commandunit == 'CT':
                    ...
                    # Split timer list and put it the child items, which were created by struct.timer
