...
#ToDos / Remarks
    ...
    # beim Setzen von Werten prüfen, ob der zu setzende Wert innerhalb des zulässigen Bereiches ist
    # Was passiert mit dem Write Response? Wird der verarbeitet, und wenn ja wie?
    # Befehle in commandset nicht in get und set unterscheiden. Das kann auch beim Parsen der Items passieren, wenn viess_read und viess_send
...
class Viessmann(SmartPlugin):
    ...
    def create_read_command(self, commandname):
        ...
        self.log_debug('Got a new read job: Command {} with value {}'.format(commandname, value))
        ...
        packetlen_expected = int(self._commandlength) + int(commandvaluebytes)
        
        #hand over built packet to send_command
        self.send_command(packet, packetlen_expected)
    ...
    def create_write_command(self, commandname, value=None):          
        ...
        packetlen_expected = int(self._commandlength)
        
        #hand over built packet to send_command
        self.send_command(packet, packetlen_expected)
        
    def send_command(self, packet, packetlen_expected):
        try:            
        ...
        if commandcode in self._params.keys() and responsedatacode == 1 and responsetypecode != 3:
            ...
            for i in range(0, len(self._params[commandcode]['items'])):
                ...
                if commandunit == 'CT':
                    ...
                    self.log_debug('Matched command {} and read transformed operating mode {} (raw value was {}) and byte length {}.'.format(commandname, value, systemschemescode, commandvaluebytes))
                elif commandunit == 'DT':
                    #device type has 8 bytes
                    devicetypebytes = rawdatabytes[:8]
                    value = self.devicetpye_decode(devicetypebytes)                 
                    self.log_debug('Matched command {} and read transformed device type {} (raw value was {}) and byte length {}.'.format(commandname, value, systemschemescode, commandvaluebytes))                    
                elif commandunit == 'SN':
                    #serial number has 7 bytes
                    serialnummerbytes = rawdatabytes[:7]
                    value = self.serialnumber_decode(serialnummerbytes)
                    self.log_debug('Matched command {} and read transformed serial number {} (raw value was {}) and byte length {}.'.format(commandname, value, systemschemescode, commandvaluebytes))
                else:
                    rawvalue = self.bytes2int(rawdatabytes, commandsignage)
                    value = self.value_transform_read(rawvalue, commandtransform)
    ...
    def serialnumber_decode(self, serialnummerbytes):
        ...
        serialnumber = 0
        for i in serialnummerbytes:
          serialnumber = serialnumber + (i - 48) * 10
        return serialnumber
